...
describe("AIResultList", () => {
	...
	describe("表示テスト", () => {
		it("複数のResultCardが表示される", () => {
			...
			expect(
				screen.getByText(/remind me "レビュー" friday at 2pm/),
			).toBeInTheDocument();
		});
		...
		it("空状態メッセージが表示される", () => {
			...
			expect(
				screen.getByText(/リマインダーを生成するには/),
			).toBeInTheDocument();
		});
	...
	describe("loading skeleton", () => {
		it("isLoading=trueでskeletonが表示される", () => {
			render(
				<AIResultList results={null} onCopy={() => {}} isLoading={true} />,
			);
	...
	describe("error boundary", () => {
		...
		it("errorがある場合、role=alertが設定される", () => {
			render(<AIResultList results={null} onCopy={() => {}} error="エラー" />);
