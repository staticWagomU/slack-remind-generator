...
describe("AIResultCard", () => {
	...
	describe("表示テスト", () => {
		it("RemindCommand 1件が表示される", () => {
			render(<AIResultCard command={mockCommand} confidence={0.95} onCopy={() => {}} />);
			
			expect(screen.getByText(/remind me "ミーティング" tomorrow at 10am/)).toBeInTheDocument();
		});

		it("confidence scoreがbadgeで表示される", () => {
			render(<AIResultCard command={mockCommand} confidence={0.85} onCopy={() => {}} />);
		...
		it("低信頼度時warning表示がある", () => {
			render(<AIResultCard command={mockCommand} confidence={0.6} onCopy={() => {}} />);
			
			expect(screen.getByText(/信頼度が低いため/)).toBeInTheDocument();
		});

		it("高信頼度時warning表示がない", () => {
			render(<AIResultCard command={mockCommand} confidence={0.9} onCopy={() => {}} />);
	...
	describe("confidence badge表示", () => {
		...
		it("中信頼度(>=0.6)は黄色", () => {
			render(<AIResultCard command={mockCommand} confidence={0.7} onCopy={() => {}} />);
		...
		it("低信頼度(<0.6)は赤色", () => {
			render(<AIResultCard command={mockCommand} confidence={0.5} onCopy={() => {}} />);
	...
	describe("インタラクション", () => {
		it("コピーボタンクリックでonCopyが呼ばれる", async () => {
			...
			render(<AIResultCard command={mockCommand} confidence={0.95} onCopy={handleCopy} />);
			
			const button = screen.getByRole("button", { name: /コピー/i });
			await user.click(button);
			
			expect(handleCopy).toHaveBeenCalledWith('remind me "ミーティング" tomorrow at 10am');
		});
	});
